...
![イメージ画像](./images/slack.png)

### キーポイント

1. Mac 前提であるが、launchd で定期的にファイルの実行を行える。
2. **`codex exec` の非対話モードでセッションを指定することができる。**
3. プロンプトは Skills を使用するように示して固定し、各プロジェクトにおいて Skills を定めることで運用体制をプロジェクトごとに細かく定める。
